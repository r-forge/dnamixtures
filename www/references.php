<h1>References</h1>
<div class="references">
  <ul>
    <li>
      <dl>
	<dt id="cowell-etal-15"><div style="float:left;width=20%">
	    <a href = "https://doi.org/10.1111/rssc.12071">
	      <span class="papertitle">Analysis of forensic DNA
	      mixtures with artefacts.</span><br/>
	      Robert G. Cowell, Therese Graversen, Steffen Lauritzen,
	      and Julia Mortera
	      <br/>
	      Journal of the Royal Statistical Society, series C. Volume
	      64, Issue 1, 1-48, 2015
	    </a>
	  </div>
	  <div style="float:right"><a href="bibtex/cowell-etal-15.txt"><img src="graphics/icon_bibtex_32x32.png"
					       title="Download BibTeX citation"/></a>
	  </div>
	  <div style="clear:both"></div>
	</dt>
	<dd class="paperdescription"><p>The paper was presented to and
	discussed at the RSS at 11 June 2014 and is published with
	contributed discussions as well as the authors' response to
	the discussion.</p>
	  <ul>
	    <li>The paper presents a statistical model for DNA
	    mixtures; <span class="rpkg">DNAmixtures</span> implements
	    this model. </li>
	    <li>All analyses are performed using <span class="rpkg">DNAmixtures</span> and are
	      documented as a short tutorial in the supporting
	      information<br/> <a href = "bibtex/rssc12071-sup-0001-suppinfo.pdf" class="papertitle">
		Case analysis using the DNAmixtures package</a> by Therese Graversen.</li>
	  </ul>
	</dd>
      </dl>
    </li>
    <li>
      <dl>
	<dt id="graversen-lauritzen-15"><div style="float:left;width=20%">
	    <a href = "http://dx.doi.org/10.1007/s11222-014-9451-7">
	      <span class="papertitle">Computational aspects of DNA mixture analysis.</span><br/>
	      Therese Graversen and Steffen Lauritzen.<br/>
	      Statistics and Computing, Volume 25, Issue 3, 527-541. 2015.</a>
	  </div>
	  <div style="float:right"><a href="bibtex/graversen-lauritzen-15.txt"><img src="graphics/icon_bibtex_32x32.png"
					       title="Download BibTeX citation"/></a>
	  </div>
	  <div style="clear:both"></div>
	</dt>
	<dd class="paperdescription">
	  <p>The paper introduces generally applicable methodology for</p>
	  <ul>
	    <li>efficient exact summation over unknown genotypes.</li>
	    <li>efficient exact computation of the likelihood function.</li>
	    <li>easy evaluation of various marginal and conditional
	    distributions of peak heights.</li>
	    <li>statistical methodology for systematic scrutiny of DNA mixture models.</li>
	  </ul>
	</dd>
      </dl>
    </li>

    <li>
      <dl>
	<dt id="graversen-14"><div style="float:left;width=20%">
	    <a href = "http://ora.ox.ac.uk/objects/uuid:4c3bfc88-25e7-4c5b-968f-10a35f5b82b0">
	      <span class="papertitle">Statistical and Computational Methodology for the Analysis of Forensic DNA Mixtures with Artefacts.</span><br/>
	      Therese Graversen.<br/>
	      DPhil. University of Oxford. 2014.</a>
	  </div>
	  <div style="float:right"><a href="bibtex/graversen-14.txt"><img src="graphics/icon_bibtex_32x32.png"
					       title="Download BibTeX citation"/></a>
	  </div>
	  <div style="clear:both"></div>
	</dt>
	<dd class="paperdescription">
	  <p>This Doctoral thesis is freely available from the Oxford
	  Bodleian Libraries and</p>
	  <ul>
	    <li>presents an in-depth discussion of the
	      methodology.</li>
	    <li>illustrates how a statistical framework may be exploited
	      to address a wide range of questions relating to the
	      analysis of a DNA mixture.
	    </li>
	    <li>gives an overview of the <span class="rpkg">DNAmixtures package</span>.</li>
	  </ul>
	</dd>
      </dl>
    </li>
  </ul>
</div>
